...
class LocationSuggestionService {
  ...
  static Future<List<LocationSuggestion>> searchLocationSuggestions(
    ...
    try {
      final onlineSuggestions = await _fetchOnlineSuggestions(cleanQuery).timeout(timeout);
    ...
    for (final m in offlineMatches) {
      ...
      if (!seenTitles.contains(key)) {
        seenTitles.add(key);
        results.add(LocationSuggestion.fromOfflineLocation(
          name: name,
          state: state,
          type: type,
        ));
  ...
  static Future<List<LocationSuggestion>> _fetchOnlineSuggestions(String query) async {
    try {
      ...
      final request = await _httpClient.getUrl(uri).timeout(const Duration(milliseconds: 800));
      request.headers.set('User-Agent', 'SalonQueueApp/1.0 (contact: support@salonqueue.app)');
  ...
  static Future<Map<String, double>> geocodeAddress({
    ...
    final fallbackCoords = getEstimatedCoordinates(city.isNotEmpty ? city : district, state);
    return {
  ...
  static ({double latitude, double longitude}) getEstimatedCoordinates(String cityOrDistrict, String state) {
    ...
    if (clean.contains('mumbai') || clean.contains('bandra') || clean.contains('andheri')) return (latitude: 19.0760, longitude: 72.8777);
    if (clean.contains('angul')) return (latitude: 20.8398, longitude: 85.1013);
    if (clean.contains('bhubaneswar')) return (latitude: 20.2961, longitude: 85.8245);
    if (clean.contains('cuttack')) return (latitude: 20.4625, longitude: 85.8828);
    if (clean.contains('bangalore') || clean.contains('bengaluru') || clean.contains('koramangala') || clean.contains('indiranagar')) return (latitude: 12.9716, longitude: 77.5946);
    if (clean.contains('delhi') || clean.contains('connaught') || clean.contains('dwarka')) return (latitude: 28.6139, longitude: 77.2090);
    if (clean.contains('hyderabad') || clean.contains('secunderabad')) return (latitude: 17.3850, longitude: 78.4867);
    ...
    if (clean.contains('patna')) return (latitude: 25.5941, longitude: 85.1376);
    if (clean.contains('chandigarh')) return (latitude: 30.7333, longitude: 76.7794);
    if (clean.contains('nagpur')) return (latitude: 21.1458, longitude: 79.0882);
    if (clean.contains('indore')) return (latitude: 22.7196, longitude: 75.8577);
    if (clean.contains('kochi') || clean.contains('cochin')) return (latitude: 9.9312, longitude: 76.2673);
    if (clean.contains('guwahati')) return (latitude: 26.1445, longitude: 91.7362);

    // State Center Fallbacks
    if (cleanState.contains('odisha') || cleanState.contains('orissa')) return (latitude: 20.9517, longitude: 85.0985);
    if (cleanState.contains('maharashtra')) return (latitude: 19.7515, longitude: 75.7139);
    if (cleanState.contains('karnataka')) return (latitude: 15.3173, longitude: 75.7139);
    if (cleanState.contains('tamil nadu')) return (latitude: 11.1271, longitude: 78.6569);
    if (cleanState.contains('gujarat')) return (latitude: 22.2587, longitude: 71.1924);
    if (cleanState.contains('rajasthan')) return (latitude: 27.0238, longitude: 74.2179);
    if (cleanState.contains('uttar pradesh')) return (latitude: 26.8467, longitude: 80.9462);
    if (cleanState.contains('west bengal')) return (latitude: 22.9868, longitude: 87.8550);
    if (cleanState.contains('bihar')) return (latitude: 25.0961, longitude: 85.3131);
    if (cleanState.contains('punjab')) return (latitude: 31.1471, longitude: 75.3412);
    if (cleanState.contains('haryana')) return (latitude: 29.0588, longitude: 76.0856);
    if (cleanState.contains('kerala')) return (latitude: 10.8505, longitude: 76.2711);
