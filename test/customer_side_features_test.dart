...
void main() {
  group('1. Customer Salon Card & Images Tests', () {
    testWidgets('SalonCard renders cover image and verified badge when present', (tester) async {
      final verifiedSalon = Salon(
        ...
        isVerified: true,
        galleryImages: ['https://example.com/photo1.jpg', 'https://example.com/photo2.jpg'],
        services: const [
          SalonService(id: 's1', salonId: 'salon-v1', name: 'Haircut', category: 'Hair', price: 150, durationMinutes: 20),
        ],
      );
    ...
    testWidgets('SalonDetailsScreen displays Owner Card, Verified badge, and Gallery', (tester) async {
      final salon = Salon(
        id: 'salon-d1',
        ...
        services: const [
          SalonService(id: 'srv1', salonId: 'salon-d1', name: 'Fade Cut', category: 'Hair', price: 200, durationMinutes: 25),
        ],
      );

      await tester.pumpWidget(
        MaterialApp(
          home: SalonDetailsScreen(salon: salon),
        ),
      );
      await tester.pumpAndSettle();
  ...
  group('2. Customer Notification System Tests', () {
    test('Customer receives notification when joining queue and when called', () async {
      ...
      expect(notifs.any((n) => n.title.contains('Queue Joined Successfully')), isTrue);
      ...
      expect(updatedNotifs.any((n) => n.title.contains('Your Turn Has Arrived')), isTrue);
    });

    testWidgets('CustomerNotificationsScreen renders tabs and notification list', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
  ...
  group('3. Support Query Resolution -> Customer Notification Flow', () {
    test('Support ticket resolution automatically creates customer notification', () async {
      ...
      await supportRepo.updateTicketStatus(
        ...
        adminResponse: 'The estimated calculation has been adjusted for peak times.',
      );

      final notifs = await notifRepo.fetchNotifications(customerId);
      final resolvedNotif = notifs.firstWhere(
        (n) => n.type == NotificationType.supportResolved && n.relatedId == 'cust-tick-55',
      );
  ...
  group('4. Four Interactive Feature Cards Tests', () {
    testWidgets('BenefitItem responds to tap events with ripple', (tester) async {
      bool tapped = false;
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
    ...
    testWidgets('SecurityPrivacyScreen displays security pillars and account actions', (tester) async {
    ...
    testWidgets('EasyBookingScreen renders title and instructions banner', (tester) async {
      final authService = AuthService(null);
