...
void main() {
  ...
  group('Atomic Token Allocation & Concurrent Safety Tests', () {
    ...
    test('Single customer joining receives token #1 and preserves all service data', () async {
    ...
    test('Sequential joins to the same salon receive strictly incrementing tokens', () async {
      final queueRepo = QueueRepository();
    ...
    test('Simultaneous / concurrent joins to the same salon never produce duplicate tokens', () async {
      final queueRepo = QueueRepository();
      ...
      expect(uniqueTokens.length, equals(count), reason: 'All concurrent tokens must be unique');
    ...
    test('Owner walk-in customer creation (null customerId) receives valid token', () async {
      final queueRepo = QueueRepository();
    ...
    test('QueueRepository clearLocalCache resets in-memory sequence counters', () async {
      final queueRepo = QueueRepository();
