...
void main() {
  ...
  group('Service CRUD & Realtime Stream Tests', () {
    ...
    test('updateService modifies price, duration and active status', () async {
      ...
      final activeOnly = await salonRepo.fetchServices(
        salonId,
        onlyActive: true,
      );
      ...
      final allServices = await salonRepo.fetchServices(
        salonId,
        onlyActive: false,
      );
  ...
  group('Customer Queue Joining & Status Flow Tests', () {
    test(
      'joinQueue calculates total price and duration from real services',
      () async {
        ...
        expect(
          ticket.serviceNames,
          containsAll(['Classic Haircut', 'Beard Trim']),
        );
    ...
    test(
      'updateTicketStatus progresses ticket through full lifecycle',
      () async {
        final queueRepo = QueueRepository();
        ...
        final postCompleteQueue = await queueRepo.fetchLiveQueueForSalon(
          salonId,
        );
        expect(
          postCompleteQueue.any(
            (t) => t.id == ticket.id && t.status == QueueStatus.inChair,
          ),
          isFalse,
        );
    ...
    test(
      'streamTicket receives realtime completed state when owner finishes service',
      () async {
        final queueRepo = QueueRepository();
        ...
        expect(
          streamEvents.any((t) => t.status == QueueStatus.completed),
          isTrue,
        );
        final finalEvent = streamEvents.lastWhere(
          (t) => t.status == QueueStatus.completed,
        );
    ...
    test(
      'fetchCompletedTicketsForSalon returns completed tickets and calculates accurate revenue',
      () async {
        final queueRepo = QueueRepository();
        ...
        final completedTickets = await queueRepo.fetchCompletedTicketsForSalon(
          salonId,
        );
        ...
        final allCompleted = await queueRepo.fetchCompletedTicketsForSalon(
          salonId,
        );
        expect(allCompleted.length, equals(2));
        final totalRevenue = allCompleted.fold<double>(
          0.0,
    ...
    test(
      'fetchLatestTicketForCustomer retrieves recent completed ticket for My Queue history',
      () async {
        final queueRepo = QueueRepository();
