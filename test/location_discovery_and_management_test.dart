...
void main() {
  ...
  group('LocationSuggestionService Tests', () {
    ...
    test('fromNominatim parses OpenStreetMap JSON correctly', () {
      final osmJson = {
        'lat': '20.2961',
        'lon': '85.8245',
        'display_name': 'Saheed Nagar, Bhubaneswar, Khordha, Odisha, 751007, India',
        'address': {
          'suburb': 'Saheed Nagar',
          'city': 'Bhubaneswar',
          'county': 'Khordha',
    ...
    test('searchLocationSuggestions finds offline Indian locations', () async {
      final results = await LocationSuggestionService.searchLocationSuggestions('Angul');
      expect(results.isNotEmpty, isTrue);
      final match = results.firstWhere((r) => r.title.toLowerCase().contains('angul'));
  ...
  group('SalonRepository Location & 10km Recommendation Tests', () {
    test('updateSalonLocation persists coordinates and location fields', () async {
  ...
  group('Owner SalonLocationScreen Widget Test', () {
    testWidgets('Owner can edit and save State, District, City, Address, Pincode', (tester) async {
      ...
      expect(find.text('Salon location updated successfully!'), findsOneWidget);
    });
  });

  group('Customer Location Selector & Salon Details Navigation Test', () {
    testWidgets('Customer sees location selector and can drilldown or search', (tester) async {
      final authService = AuthService(null);
    ...
    testWidgets('SalonDetailsScreen displays owner profile, service menu, and booking options', (tester) async {
      const salon = Salon(
