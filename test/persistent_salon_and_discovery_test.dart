...
void main() {
  group('Persistent Salon Data, Media & Cross-Device Location Discovery Suite', () {
    ...
    test('TEST 1, 2, 3, 4: Full Owner persistence (profile, cover, gallery, timings, chairs, location) across sessions', () async {
      ...
      await salonRepo.updateOwnerProfile(
        ...
        ownerAvatarUrl: 'https://images.unsplash.com/photo-owner-avatar-123.jpg',
      );
      ...
      expect(updatedA.coverImageUrl, 'https://images.unsplash.com/photo-cover-123.jpg');
    ...
    test('TEST 5, 6, 7: Location-based customer discovery matches state, city, district & keywords across devices', () async {
      final sampleSalons = [
        Salon(
          id: 'salon-1',
          ownerId: 'owner-1',
      ...
      final angulResults = sampleSalons.where((s) =>
          s.city.toLowerCase().contains('angul') || s.district.toLowerCase().contains('angul')).toList();
      ...
      final keywordMatch = sampleSalons.where((s) =>
          s.ownerName != null && s.ownerName!.toLowerCase().contains('bazirakesh')).toList();
      expect(keywordMatch.length, 1);
      expect(keywordMatch.first.name, 'Rakesh Kurya Salon');

      // 3. Customer searches "Pune" -> matches Pune salon
      final cityMatch = sampleSalons.where((s) =>
          s.city.toLowerCase().contains('pune') || s.district.toLowerCase().contains('pune')).toList();
      expect(cityMatch.length, 1);
      expect(cityMatch.first.name, 'My Salon & Spa Pune');
    });

    test('TEST 8 & 9: Customer profile image upload returns persistent valid URI format', () async {
    ...
    test('TEST 10: Empty state when no salon registered in queried city', () async {
