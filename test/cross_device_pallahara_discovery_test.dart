...
void main() {
  ...
  group('Canonical Cross-Device Salon Discovery Tests (Pallahara / Odisha / Spa)', () {
    test('Location normalization handles mixed case, whitespace and spacing identically', () {
      expect(SalonRepository.normalizeLocation(' Pallahara '), equals('pallahara'));
      expect(SalonRepository.normalizeLocation('PALLAHARA'), equals('pallahara'));
      expect(SalonRepository.normalizeLocation('Angul  '), equals('angul'));
      expect(SalonRepository.normalizeLocation(' Odisha  '), equals('odisha'));
    });

    test('IndiaLocations correctly resolves Angul district for Pallahara village', () {
      final district = IndiaLocations.resolveDistrictForCity('Pallahara', 'Odisha');
      expect(district, equals('Angul'));
    });

    test('Owner creates Spa in Pallahara, Customer searches Odisha/Angul/Pallahara with "spa" and discovers it', () async {
      ...
      expect(customerSalons.isNotEmpty, isTrue, reason: 'Customer must discover Spa in Pallahara');
    ...
    test('Realtime queue toggle update reflects in salon discovery', () async {
      ...
      final offlineFetch = await repo.fetchSalons(state: 'Odisha', city: 'Pallahara');
      ...
      final onlineFetch = await repo.fetchSalons(state: 'Odisha', city: 'Pallahara');
